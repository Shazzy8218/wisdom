...
import { Loader2, Radio, SlidersHorizontal, RefreshCw, Shield, Eye, Zap } from "lucide-react";
...
export default function LearnFeed() {
  ...
  const [mode, setMode] = useState<FeedMode>("decoder");
  ...
  const generateBatch = useCallback(async (count: number) => {
    ...
    for (let i = 0; i < count; i++) {
      try {
        ...
        if (card && card.id) newCards.push(card);
      } catch (err) {
        console.error("Failed to generate feed card:", err);
        if (i === 0) setError(err instanceof Error ? err.message : "Failed to generate content");
        break;
      }
    }

    if (newCards.length > 0) { setCards(prev => [...prev, ...newCards]); setError(null); }
    setLoading(false);
    setInitialLoading(false);
      ...
      <div className="flex-shrink-0 px-5 pt-4 pb-3 bg-background/90 backdrop-blur-xl z-10 border-b border-border/50">
        <div className="flex items-center justify-between">
          <div>
            <div className="flex items-center gap-2">
              ...
              <p className="text-[9px] font-black uppercase tracking-[0.3em] text-primary">Phenomenon Decoder</p>
            </div>
            <p className="text-caption text-muted-foreground mt-0.5">Strategic readouts · Pattern recognition · Reality amplification</p>
          </div>
        ...
        {showModePanel && (
          <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} className="mt-3 space-y-2">
            {(Object.keys(MODE_CONFIG) as FeedMode[]).map(m => (
              <button key={m} onClick={() => { setMode(m); setShowModePanel(false); }}
                ...
                <span className={mode === m ? "text-primary" : "text-muted-foreground"}>{MODE_CONFIG[m].icon}</span>
                <div>
                  <p className="text-caption font-semibold text-foreground">{MODE_CONFIG[m].label}</p>
                  <p className="text-[10px] text-muted-foreground">{MODE_CONFIG[m].desc}</p>
                </div>
              </button>
            ))}
          </motion.div>
          ...
          <div className="min-h-[50vh] snap-start flex items-center justify-center">
            <div className="flex flex-col items-center gap-3 text-center px-8">
              ...
              <button onClick={() => generateBatch(2)}
                ...
                <RefreshCw className="h-4 w-4" /> Reconnect signal
              </button>
            </div>
          </div>
        )}
        ...
        {loading && !initialLoading && (
          <div className="min-h-[50vh] snap-start flex items-center justify-center">
            <div className="flex flex-col items-center gap-3">
              ...
              <p className="text-caption text-muted-foreground">Decoding new phenomena...</p>
            </div>
          </div>
        )}
        ...
        {!loading && cards.length > 0 && (
          <div className="snap-start flex items-center justify-center py-12">
            <button onClick={() => generateBatch(2)} disabled={loading}
              className="glass-card px-6 py-3 flex items-center gap-2 text-caption font-semibold text-primary hover:border-primary/30 transition-all">
              <Radio className="h-4 w-4" /> Decode More Phenomena
            </button>
          </div>
        )}
      </div>
