...
import { Bookmark, BookmarkCheck, Share2, CheckCircle2, AlertTriangle, Zap, ChevronDown, Eye } from "lucide-react";
...
import { TrendMapViz, InfluenceWebViz, SourceComparisonViz, ContrastingViewsViz } from "@/components/feed/PatternVisualization";
...
function VisualBlock({ card }: { card: FeedCardType }) {
  ...
  if (card.type === "source-comparison" && card.sourceStreams?.length) {
    return <SourceComparisonViz card={card} />;
  }

  // Reality check contrasting views
  if (card.type === "reality-check" && card.contrastingViews) {
    return <ContrastingViewsViz card={card} />;
  }
